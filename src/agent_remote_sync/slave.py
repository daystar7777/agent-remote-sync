...
class SlaveState:
    ...
    def login(
        ...
        self.log(f"Accepted master session from {client} scopes={','.join(granted_scopes)}")
...
class SlaveHandler(BaseHTTPRequestHandler):
    ...
    def run_guarded(self, handler: Any, *, authenticated: bool = False) -> None:
        ...
        try:
            try:
                self.server.state.security.check_rate(ip, authenticated=authenticated)
                handler()
    ...
    def handle_upload_status(self, payload: dict[str, Any]) -> None:
        ...
        part, _ = partial_paths(self.server.state.root, path_text)
        existing_type = None
        if target.exists():
