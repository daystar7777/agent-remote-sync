            ...
            while True:
                ...
                try:
                    ...
                    if attempts == 0 and exc.code in RECOVERABLE_UPLOAD_FINISH_ERRORS:
                        attempts += 1
                        console_print(f"remote partial failed validation; retrying {item['target']} from the start")
                        continue
                    raise
...
def tell(
    ...
    *,
    ...
    handoff = create_handoff(
        ...
        task=task,
        from_model=from_name or "agentremote-local",
        to_model=to_name,
...
def handoff(
    ...
    *,
    ...
    try:
        ...
        cleanup_paths = [str(item["target"]) for item in transfer.get("files", []) if item.get("target")]
        _cleanup_handoff_files(
            host,
            port,
            password,
            token,
...
def _cleanup_handoff_files(
    ...
    *,
    ...
    try:
        ...
        # Only delete concrete files uploaded by this handoff. Deleting the
        # transfer root can remove pre-existing remote directory contents.
...
def local_scope(path: Path, root: Path | None = None) -> tuple[Path, str]:
    ...
    if not target.exists():
        raise AgentRemoteError(404, "not_found", f"Local path not found: {path}")
    try:
...
def resolve_conflicts(conflicts: list[str], overwrite: bool, side: str) -> bool:
    ...
    if not sys.stdin.isatty():
        raise AgentRemoteError(409, "conflicts", "Conflicts found; rerun with --overwrite")
    try:
        answer = input("Overwrite these files? [y/N] ").strip().lower()
    except EOFError as exc:
        raise AgentRemoteError(409, "conflicts", "Conflicts found; rerun with --overwrite") from exc
    if answer not in ("y", "yes"):
        raise AgentRemoteError(409, "conflicts", "Transfer cancelled because of conflicts")
    return True
