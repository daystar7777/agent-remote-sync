...
PARTIAL_DIR_NAME = ".agentremote_partial"
INBOX_DIR_NAME = ".agentremote_inbox"
HANDOFF_DIR_NAME = ".agentremote_handoff"
STATE_DIR_NAME = ".agentremote"
RESERVED_DIR_NAMES = {PARTIAL_DIR_NAME, INBOX_DIR_NAME, HANDOFF_DIR_NAME, STATE_DIR_NAME}
AUTH_ITERATIONS = 200_000


class AgentRemoteError(Exception):
    def __init__(
        self,
        status: int,
        code: str,
...
class TransferJob:
    ...
    def as_dict(self) -> dict[str, Any]:
        observed_at = self.finished_at or time.time()
        elapsed = max(0.0, observed_at - self.started_at)
        speed_bytes_per_second = self.done_bytes / elapsed if elapsed > 0 else 0.0
        remaining_bytes = max(0, self.total_bytes - self.done_bytes)
        eta_seconds = (
            remaining_bytes / speed_bytes_per_second
            if remaining_bytes > 0 and speed_bytes_per_second > 0
            else None
        )
        return {
            "id": self.id,
    ...
    def raise_if_cancelled(self) -> None:
        if self.cancel_requested:
            raise AgentRemoteError(499, "cancelled", "Transfer was cancelled")
...
def read_json_body(handler: Any) -> dict[str, Any]:
    ...
    try:
        ...
        raise AgentRemoteError(400, "bad_content_length", "Invalid Content-Length") from exc
    if length <= 0:
        return {}
    if length > MAX_JSON_BODY:
        drain_request_body(handler, length, MAX_JSON_BODY + 1)
        raise AgentRemoteError(413, "json_too_large", "JSON request body is too large")
    data = handler.rfile.read(length)
    try:
        payload = json.loads(data.decode("utf-8"))
    except json.JSONDecodeError as exc:
        raise AgentRemoteError(400, "bad_json", "Request body must be valid JSON") from exc
    if not isinstance(payload, dict):
        raise AgentRemoteError(400, "bad_json", "Request body must be a JSON object")
    return payload
...
def send_error(handler: Any, exc: Exception) -> None:
    ...
    if isinstance(exc, AgentRemoteError):
...
def storage_error(exc: OSError, action: str = "file operation") -> AgentRemoteError:
    code = getattr(exc, "errno", None)
    if code == errno.ENOSPC:
        return AgentRemoteError(507, "insufficient_storage", f"{action} failed because disk space is exhausted")
    if code in (errno.EACCES, errno.EPERM):
        return AgentRemoteError(403, "permission_denied", f"{action} failed because permission was denied")
    if hasattr(errno, "EROFS") and code == errno.EROFS:
        return AgentRemoteError(403, "read_only_filesystem", f"{action} failed because the filesystem is read-only")
    if code == errno.ENAMETOOLONG:
        return AgentRemoteError(400, "name_too_long", f"{action} failed because a filename is too long")
    if code == errno.ENOTDIR:
        return AgentRemoteError(400, "not_directory", f"{action} failed because a path segment is not a directory")
    if code in (errno.EMFILE, errno.ENFILE):
        return AgentRemoteError(503, "file_resource_exhausted", f"{action} failed because file handles are exhausted")
    return AgentRemoteError(500, "storage_error", f"{action} failed: {exc}")
...
def clean_rel_path(path_text: str | None, *, allow_reserved: bool = False) -> str:
    ...
    if "\x00" in text:
        raise AgentRemoteError(400, "bad_path", "Path contains a null byte")
    ...
    for part in pure.parts:
        if part in ("", "/", "."):
            continue
        if part == "..":
            raise AgentRemoteError(400, "bad_path", "Path traversal is not allowed")
        if ":" in part:
            raise AgentRemoteError(400, "bad_path", "Drive-style paths are not allowed")
        part = normalize_wire(part)
        if contains_control(part):
            raise AgentRemoteError(400, "bad_path", "Control characters are not allowed in paths")
        if not allow_reserved and part in RESERVED_DIR_NAMES:
            raise AgentRemoteError(400, "reserved_path", "agent-remote-sync partial state is reserved")
        parts.append(part)
    return "/" + "/".join(parts)
...
def safe_name(name: str) -> str:
    ...
    if not name or name in (".", ".."):
        raise AgentRemoteError(400, "bad_name", "Name is not valid")
    if "/" in name or "\\" in name or "\x00" in name or ":" in name:
        raise AgentRemoteError(400, "bad_name", "Name must be a single path segment")
    if contains_control(name):
        raise AgentRemoteError(400, "bad_name", "Control characters are not allowed in names")
    if name in RESERVED_DIR_NAMES:
        raise AgentRemoteError(400, "reserved_name", "Name is reserved by agent-remote-sync")
    return name
...
def ensure_inside(root: Path, resolved: Path) -> None:
    try:
        ...
        raise AgentRemoteError(403, "path_escape", "Path escapes the configured root") from exc
...
def list_dir(root: Path, path_text: str | None) -> dict[str, Any]:
    ...
    if not path.is_dir():
        raise AgentRemoteError(400, "not_directory", "Path is not a directory")
    entries = []
    for child in sorted(
