...
MTIME_TOLERANCE = 1.0
RECOVERABLE_UPLOAD_FINISH_ERRORS = {"hash_mismatch", "size_mismatch"}
...
def sync_plan_push(
    ...
    *,
    ...
    local_entries = local_index(source_root, exclude_patterns=patterns, excluded=excluded)
    remote_entries = remote_index(remote, remote_dir)
    local_dirs = local_dir_index(source_root, exclude_patterns=patterns)
    ...
    plan["excludePatterns"] = patterns
    plan["excluded"] = sorted(excluded, key=lambda item: item["rel"])
    plan["summary"]["excluded"] = len(excluded)
    if compare_hash:
...
def sync_push(
    ...
    *,
    ...
    plan = sync_plan_push(root, source_root, remote_dir, remote, compare_hash=compare_hash, exclude_patterns=exclude_patterns)
...
def resolve_local_sync_root(local_root: Path, local_path: Path) -> Path:
    ...
    if not target.exists():
        raise AgentRemoteError(404, "not_found", f"Local sync path not found: {local_path}")
    if not target.is_dir():
        raise AgentRemoteError(400, "sync_requires_directory", "sync currently requires a directory")
    return target
    ...
    *,
    ...
    for current, dirs, files in os.walk(root, followlinks=False, onerror=walk_error):
        ...
        for dirname in dirs:
            ...
            pattern = exclude_match(rel, is_dir=True, patterns=patterns)
            if dirname in RESERVED_DIR_NAMES or pattern:
                if pattern:
                    append_excluded(excluded, rel, "dir", pattern)
                continue
        ...
        for filename in files:
            ...
            if child.is_symlink():
                continue
            rel = child.relative_to(root).as_posix()
            pattern = exclude_match(rel, is_dir=False, patterns=patterns)
            if pattern:
                append_excluded(excluded, rel, "file", pattern)
                continue
            try:
                stat = child.stat()
            except OSError as exc:
...
def local_dir_index(root: Path, *, exclude_patterns: Iterable[str] | None = None) -> dict[str, dict[str, Any]]:
    root = root.resolve()
    entries: dict[str, dict[str, Any]] = {}
    if not root.exists():
        return entries
    ...
    for current, dirs, _files in os.walk(root, followlinks=False, onerror=walk_error):
        ...
        for dirname in dirs:
            ...
            if child.is_symlink():
                continue
            rel = child.relative_to(root).as_posix()
            if dirname in RESERVED_DIR_NAMES or exclude_match(rel, is_dir=True, patterns=patterns):
                continue
...
def normalize_exclude_patterns(patterns: Iterable[str] | None) -> list[str]:
    normalized: list[str] = []
...
def exclude_match(rel: str, *, is_dir: bool, patterns: Iterable[str] | None) -> str | None:
    ...
    for pattern in normalize_exclude_patterns(patterns):
        ...
        if dir_only:
            if has_slash:
                if path == body or path.startswith(body + "/") or fnmatchcase(path, body) or fnmatchcase(path, body + "/*"):
                    return pattern
            elif any(fnmatchcase(part, body) for part in parts):
                return pattern
            continue
        if has_slash:
            if path == body or path.startswith(body + "/") or fnmatchcase(path, body) or fnmatchcase(path, body + "/*"):
...
def append_excluded(excluded: list[dict[str, str]] | None, rel: str, item_type: str, pattern: str) -> None:
    if excluded is None:
        return
    marker = rel.rstrip("/") + "/" if item_type == "dir" else rel.rstrip("/")
    excluded.append({"rel": marker, "type": item_type, "pattern": pattern})
...
def remote_index(remote: RemoteClient, remote_dir: str, *, missing_ok: bool = True) -> dict[str, dict[str, Any]]:
    ...
    if not stat.get("exists"):
        if not missing_ok:
            raise AgentRemoteError(404, "not_found", f"Remote sync path not found: {remote_dir}")
        return {}
    if stat["entry"]["type"] != "dir":
        raise AgentRemoteError(400, "sync_requires_directory", "remote sync path must be a directory")
    entries: dict[str, dict[str, Any]] = {}
...
def remote_relative(base: str, child: str) -> str:
    ...
    raise AgentRemoteError(400, "bad_tree", "Remote tree returned a path outside the requested sync root")
...
def resolve_delete_candidates(candidates: list[dict[str, Any]], delete: bool, side: str) -> bool:
    ...
    if answer not in ("y", "yes"):
        raise AgentRemoteError(409, "delete_cancelled", "Sync delete was cancelled")
    return True
...
def delete_local_items(target_root: Path, candidates: list[dict[str, Any]], logger: TransferLogger) -> None:
    ...
    for item in candidates:
        ...
        try:
            ...
            raise AgentRemoteError(403, "path_escape", "Delete candidate escapes sync root") from exc
        if target.is_dir():
            raise AgentRemoteError(400, "delete_candidate_not_file", "Sync delete candidates must be files")
    ...
    for item in items:
        ...
        while True:
            ...
            try:
                ...
                if attempts == 0 and exc.code in RECOVERABLE_UPLOAD_FINISH_ERRORS:
                    attempts += 1
                    print(f"remote partial failed validation; retrying {item['target']} from the start")
                    continue
                raise
