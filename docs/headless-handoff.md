...
agent-remote-sync push lab ./project /incoming/project
...
- record sessions/logs under `.agent_remote_sync`.
...
```powershell
agent-remote-sync connect XXX 100.64.1.20
```

Aliases are normalized with a `::` prefix. `agent-remote-sync connect lab ...` creates
...
```text
AIMemory/agent_remote_sync_hosts/lab.md
```
...
```powershell
agent-remote-sync tell XXX "Run tests for /incoming/project and report failures." --path /incoming/project
...
```powershell
agent-remote-sync handoff XXX ./LLL "Use the uploaded file to do ZZZ and report back."
```

`agent-remote-sync handoff` is the preferred high-level command for user requests such as
...
The receiving side stores the manifest under `.agent_remote_sync_inbox`. A local agent on
...
```powershell
agent-remote-sync inbox --claim <instruction-id>
...
lines explicitly prefixed with `agent-remote-sync-run:`:

```text
Please run the tests.
agent-remote-sync-run: python -m unittest discover -s tests
```

Run with approval:

```powershell
agent-remote-sync worker --once --execute ask
```
...
```powershell
agent-remote-sync worker --once --execute yes
```

For unattended workers, combine non-interactive execution with a narrow project
root, scoped tokens, explicit `agent-remote-sync-run:` commands, and a trusted network or
...
```powershell
agent-remote-sync handoff worker ./LLL "Do ZZZ" --auto-run --callback-alias master
```
...
1. Master runs `agent-remote-sync handoff XXX ./LLL "Do ZZZ"` when files are needed.
...
5. Slave agent runs `agent-remote-sync worker --once --execute ask` or performs the task manually.
6. Slave runs `agent-remote-sync report master <handoff-id> "<result>"`, or worker sends it through `--callback-alias`.
7. Master receives an external STATUS_REPORT handoff and displays it through
   `agent-remote-sync inbox` or future notification UI.

## Safety

Auto handoff execution is powerful. It needs explicit opt-in:

- receiver starts with `agent-remote-sync handoff receive --auto`,
- receiver claims/inspects the manifest with `agent-remote-sync worker --once`,
- commands run inside the receiver-selected project root,
- only explicit `agent-remote-sync-run:` command lines are executable,
