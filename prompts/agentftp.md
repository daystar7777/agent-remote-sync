# agentFTP Agent Prompt

You can operate agentFTP for cross-machine folder transfer.
...
```powershell
agentftp connect XXX <host-or-ip>
```
...
```powershell
agentftp connect XXX https://<host>:7171 --tls-fingerprint <sha256>
```

This prompts for the slave password once and stores a session token. agentFTP
...
Host-specific history is stored under `AIMemory/agentftp_hosts/`. Check that
...
2. Run `agentftp slave`, or `agentftp slave --tls self-signed` when HTTPS is requested.
...
1. Run `agentftp master <host> <port>`.
...
- Use `.agentftp/sessions/<id>.json` and `.agentftp/logs/*.jsonl` for detailed
...
2. For a simple file transfer, use `agentftp push <connection-name> <local> <remote-dir>`.
3. For instruction-only handoff, use `agentftp tell <connection-name> "<task>"`.
4. For file plus instruction, prefer
   `agentftp handoff <connection-name> <local> "<task>"`.
...
8. For receiver-side processing, use `agentftp worker --once` first. It dry-runs
   by default and only executes explicit `agentftp-run:` lines when
